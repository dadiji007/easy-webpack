...
const traverse = require("@babel/traverse").default;    //require（es6）模块会将结果放到default中
...
function createAsset(filename) {
  ...
  const ast = babylon.parse(content, {    //obj.parse()，将内容转化为js对象
    sourceType: "module"
  });
  ...
  //traverse，https://astexplorer.net/，获取ast中具体代码。
  //遍历当前ast（抽象语法树）
  traverse(ast, {
    //找到有 import语法 的对应节点
...
function createGraph(entry) {
  ...
  for (const asset of queue) {
    ...
    asset.dependencies.forEach(relativePath => {
      ...
      // dirname:example ，relativePath：./message.js
